...
GameState::GameState(Game &ref) : State(ref) {
    ...
    CharacterData Hocus = CharacterData("Hocus", "wizard", 125.f, 100.f, "Fire wand", "fire_wand");
    ...
    // projectile stuff
    game.getResourceManager().loadTexture("skull", "assets/skull_projectile.png");
...
void GameState::Resize(int w, int h) {
    ...
    if (hud) {
        ...
        const int lw = static_cast<int>(std::round(static_cast<float>(w) * vp.size.x));
        const int lh = static_cast<int>(std::round(static_cast<float>(h) * vp.size.y));
        hud->onResize(lw, lh);
    }
}
    ...
    if (spawnTimer > 1.5f) {
        ...
        if (rand() % 2 == 0) {
            ...
            enemies.push_back(std::make_unique<Flying_Skull>(
                game.getResourceManager().getTexture("spinning_skull"), spawnPos, projectiles, game.getResourceManager()));
        }
    }
